...
-- First, drop all dependent policies on the role column
DROP POLICY IF EXISTS "Profiles: self update (no role)" ON public.profiles CASCADE;
DROP POLICY IF EXISTS "Admins can update any profile role" ON public.profiles CASCADE;
...
-- 2. MIGRATE EXISTING ROLE DATA BEFORE DROPPING COLUMN
-- ==============================================
...
-- Now safe to remove role column from profiles table
ALTER TABLE public.profiles DROP COLUMN role;

-- 3. CREATE SECURITY DEFINER FUNCTIONS (PREVENTS RECURSIVE RLS)
-- ==============================================
...
-- 5. FIX DATABASE FUNCTIONS SECURITY (ADD SEARCH_PATH)
-- ==============================================
...
-- 6. UPDATE CRITICAL FUNCTIONS TO USE NEW ROLE SYSTEM
-- ==============================================
...
BEGIN
  ...
  -- 2. Add users (manager & worker) to profiles and roles
...
-- 7. DROP SECURITY DEFINER VIEWS (CRITICAL SECURITY ISSUE)
-- ==============================================
...
-- 8. CREATE UTILITY FUNCTIONS FOR ROLE CHECKING
-- ==============================================
