-- Security Fix 1: Add validation triggers for role assignments
...
-- Security Fix 2: Add validation for task assignments
...
-- Security Fix 3: Enhanced audit logging with more context
...
-- Security Fix 4: Enhanced rate limiting function with better tracking
...
-- Security Fix 5: Add indexes for better performance on security-related queries (without CONCURRENTLY)
CREATE INDEX IF NOT EXISTS idx_user_roles_user_id ON public.user_roles(user_id);
CREATE INDEX IF NOT EXISTS idx_user_project_role_user_project ON public.user_project_role(user_id, project_id);
CREATE INDEX IF NOT EXISTS idx_security_audit_log_user_timestamp ON public.security_audit_log(user_id, timestamp DESC);
